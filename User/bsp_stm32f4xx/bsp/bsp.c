...
void bsp_Init(void)
{
  ...
  COM_POWER_ON;
	...
	BAT_POWER_ON;
	GPS_BAT_POWER_ON;
	CAN_485_PowerComtrol_Init();

#if USE_GSM_UART_DMA  ==1
	/*串口DMA测试*/
	TM_USART_DMA_Init(USART3);
	TM_USART_DMA_InitWithStreamAndChannel(USART3,USART3_DMA_TX_STREAM,USART3_DMA_TX_CHANNEL);
#endif	

	/* mount flie system */
	result=f_mount(0, &fats);
	
  /*monitor for the board states*/
	/*period:2s*/
	tmr_monitor = TMR_Subscribe ( TRUE, 20, TMR_TYPE_100MS, ( tmr_procTriggerHdlr_t )TmrMonitorHdlr);
