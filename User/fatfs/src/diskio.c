...
//			| ((DWORD)0 >> 1);				/* Sec = 0 */
		...
		return	  ((DWORD)(RMC_d.date_yy+20) << 25)	/* Year = 2013 */
			| ((DWORD)RMC_d.date_mm << 21)				/* Month = 1 */
			| ((DWORD)RMC_d.date_dd << 16)				/* Day_m = 1*/
			| ((DWORD)RMC_d.time_hh << 11)				/* Hour = 0 */
			| ((DWORD)RMC_d.time_mm << 5)				/* Min = 0 */
			| ((DWORD)RMC_d.time_ss >> 1);				/* Sec = 0 */
}
