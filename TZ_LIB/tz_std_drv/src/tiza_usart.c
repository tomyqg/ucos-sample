...
//	}
	...
	if ( Status == SET )
	{ 
		SHELL_ReceiveHandler(USART1->DR);
	}		
}
#endif
...
void USART3_IRQHandler(void) {
	...
	ITStatus  Status;
	Status = USART_GetITStatus ( USART3, USART_IT_RXNE );
	if ( Status == SET )
	{ 
		GSM_ReceiveHandler(USART3->DR);
...
void UART4_IRQHandler(void) {
	...
	ITStatus  Status;
	Status = USART_GetITStatus ( UART4, USART_IT_RXNE );
	if ( Status == SET )
	{ 
		GPS_ReceiveHandler(UART4->DR);
