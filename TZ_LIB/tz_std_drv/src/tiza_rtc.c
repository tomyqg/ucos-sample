...
#define RTC_STATUS_REG1                 RTC_BKP_DR18 /* Status Register */
#define RTC_SOURCE_CLOCK_LSE            0x5678       /* CLOCK: LSE */
#define RTC_SOURCE_CLOCK_LSI            0x8765       /* CLOCK: LSI*/
...
uint32_t TM_RTC_Init(TM_RTC_ClockSource_t source) {
	...
	status1 = RTC_ReadBackupRegister(RTC_STATUS_REG1);
	...
	if (status == RTC_STATUS_TIME_OK) {
		...
		if (source == TM_RTC_ClockSource_Internal) {
				if(status1 != RTC_SOURCE_CLOCK_LSI){
						RCC_BackupResetCmd(ENABLE);
						RCC_BackupResetCmd(DISABLE);				
			}
			TM_RTC_Config(TM_RTC_ClockSource_Internal);
			RTC_WriteBackupRegister(RTC_STATUS_REG1, RTC_SOURCE_CLOCK_LSI);	
		}
		else
		if(source == TM_RTC_ClockSource_External){
			if(status1 != RTC_SOURCE_CLOCK_LSE){
						RCC_BackupResetCmd(ENABLE);
						RCC_BackupResetCmd(DISABLE);				
			}
			TM_RTC_Config(TM_RTC_ClockSource_External);
			RTC_WriteBackupRegister(RTC_STATUS_REG1, RTC_SOURCE_CLOCK_LSE);
		}
